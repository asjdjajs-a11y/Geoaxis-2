// GeoAxis — Noticias mineras en tiempo real con la imagen real de cada noticia.
//
// Para cada noticia:
//   1) abrimos la página de Google News (de ahí sacamos la miniatura de respaldo
//      y los parámetros para decodificar),
//   2) decodificamos hasta el sitio real del medio,
//   3) leemos el og:image del medio.
// Imagen final = la del medio  ->  o la miniatura de Google  ->  o vacío.

const ENRICH_LIMIT = 8;
const BATCH = 8;          // en paralelo (una sola tanda para entrar dentro del límite de tiempo)
...
// Descarta direcciones mal formadas o con caracteres que romperían el HTML.
...
// Pide la URL real al endpoint interno de Google usando firma y timestamp.
...
async function enrich(item) {
  ...
  try {
    ...
    if (pageRes.ok) {
      ...
      gThumb = ogImageFromHtml(html); // miniatura de respaldo
  ...
  if (realUrl) item.url = realUrl; // enlace limpio al medio
  ...
  try {
    ...
    return {
      ...
      headers: {
        ...
        'netlify-cdn-cache-control': 'public, max-age=900, stale-while-revalidate=600',
      },
    ...
    return {
      ...
      body: JSON.stringify({ error: 'No se pudieron cargar noticias reales' }),
    };
  }
};
